...
class PaintTree:
    ...
    def paint_tree(win, tree: Tree):
        ...
        if not tree.no_descendants():
            ...
            PaintTree.__paint_tree_recursive(win, tree,
    ...
    @staticmethod
    def __paint_tree_recursive(win, tree: Tree, point: Point, x_min, x_max, visited_nodes):
        # Draw root node
        ...
        # Draw lines to kids
        childs = PaintTree.__get_no_visited_childs(tree, visited_nodes)
        ...
        current = x_min
        for i in childs:
            PaintTree.__paint_tree_recursive(win, i, Point((current + available_screen) / 2,
                                                         y_kids + Dimensions.CIRCLE_RADIUS.value), current,
                                           current + available_screen, visited_nodes)
            current += 2 * available_screen

    @staticmethod
    def __get_no_visited_childs(tree, visited):
        childs = []
        for i in tree.descendants:
            if not PaintTree.__already_visited(i.root, visited):
                childs.append(i)
        return childs

    @staticmethod
    def __already_visited(root, visited_nodes):
